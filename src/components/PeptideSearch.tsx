...
export function PeptideSearch({
  ...
  const results = useMemo(() => searchPeptides(q).slice(0, 8), [q]);
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault();
        if (q.trim()) router.push(`/peptides?q=${encodeURIComponent(q.trim())}`);
      }}
    >
      <label className="sr-only" htmlFor="peptide-search">
        Search a peptide, compound or research area
      </label>
      <input
        id="peptide-search"
        value={q}
              ...
              {results.map((p) => (
                <li key={p.slug}>
                  ...
                  >
                    ...
                    <span className="text-xs text-ink-soft">{p.peptideClass}</span>
                  </Link>
                </li>
              ))}
            </ul>
